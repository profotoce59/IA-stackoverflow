...
BEGIN
	...
	exec('use [' + @database_name + '] if object_id(''Comments'') is not null update [' + @local + '].dbo.Sites set TotalComments = (select count(*) from Comments) where Id = ' + @id_string)	
	exec('use [' + @database_name + '] if object_id(''PostComments'') is not null update [' + @local + '].dbo.Sites set TotalComments = (select count(*) from PostComments) where Id = ' + @id_string)	
